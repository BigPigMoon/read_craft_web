<script lang="ts">
	...
	onMount(() => {
		api.get<Book>(`/api/book/get/${bookId}`).then((res) => {
			book = res.data;
		});

		api.get<any[]>(`/api/book/chunk/${bookId}`).then((res) => {
	...
	const nextPage = () => {
		if (nextNeedLoad) {
			api.post(`/api/book/page/${bookId}?page=${chunkPage + 1}`).then((res) => {
				prevPageCount = pageNum;

				api.get(`/api/book/chunk/${bookId}`).then((res) => {
					chunk = setChunkText(res.data[0]);
					chunkPage = res.data[1];
