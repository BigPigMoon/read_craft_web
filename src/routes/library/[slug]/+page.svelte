<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			const res = await api.get<Book>(`/api/book/get/${bookId}`);
	...
	const deleteBook = () => {
		api.delete(`/api/book/delete/${bookId}`).then((res) => {
			goto('/library');
		});
	};
</script>
...
<div class="flex w-full justify-center h-[calc(100vh-224px)]">
	<div class="card card-side w-full h-full bg-base-200">
		...
		{:else if book}
			<figure>
				<img class="h-full max-w-5xl" src="{API_URL}/api/image/{book.cover_path}" alt="witcher" />
			</figure>
