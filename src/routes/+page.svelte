<script lang="ts">
	...
	onMount(async () => {
		...
		if (!storedFolderId) {
			folderId = (await api.get<number>('/api/group/root')).data;
		} else {
	...
	const createCard = async () => {
		const createWordRes = await api.post('/api/card/create', {
			word: newCardWord,
	...
	const createFolder = async () => {
		...
		const createFolderRes = await api.post('/api/group/create', {
	...
	const selectFolder = async (folderId: number) => {
		...
		const getItemsRes = (await api.get<Item[]>(`/api/group/items/${selectedFolderId}`)).data.sort(
			sortItems
		);

		items = [...getItemsRes];

		path = (await api.get<Folder[]>(`/api/group/path/${selectedFolderId}`)).data.reverse();
	};
	...
	const deleteItemDrop = async (event: DragEvent) => {
		...
		const id = item.id;
		if (item.type === 'card') {
			await api.delete(`/api/card/delete/${id}`);
